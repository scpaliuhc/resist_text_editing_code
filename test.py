...
def test(imgfile, savepath, saveprefix, use_cpu=True, gpuid=0, use_postref=True, iter_count=200):
...
def test_model(model, dev, imgfile, savepath, saveprefix, use_postref=True, iter_count=200):
    # load selected image
    img_norm, img_orig, pil_img = transform_inputs(imgfile, 640)
    img_norm, img_orig = img_norm.to(dev), img_orig.to(dev)
    img_size = torch.tensor([pil_img.size[1], pil_img.size[0]]).unsqueeze(0)
    ...
    plt.subplot(1, 3, 1)
    plt.imshow(img_orig.data.cpu().numpy()[0].transpose(1,2,0)/2+0.5)
    plt.axis("off")
    plt.subplot(1, 3, 2)
    plt.imshow(rec_img.data.cpu().numpy()[0].transpose(1, 2, 0)/255)
    plt.axis("off")
    plt.subplot(1, 3, 3)
    plt.imshow(output_img)
    plt.axis("off")
    plt.savefig(os.path.join(savepath, f'{saveprefix}.jpg'))
    plt.close()
    with open(os.path.join(savepath, f'{saveprefix}.pkl'), mode='wb') as f:
        pickle.dump(vd, f)
